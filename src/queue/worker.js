...
import { startFeedConsumer } from './consumers/feed.consumer.js';
...
const FEED_CLEANUP_INTERVAL_MS = 24 * 60 * 60 * 1000;
...
async function main() {
  ...
  // Daily feed cleanup — publish to the queue so the prefetch: 1 consumer
  // ensures only one cleanup batch runs at a time.
  setInterval(() => {
    publish(EXCHANGES.EVENTS, ROUTING_KEYS.FEED_CLEANUP_REQUESTED, { source: 'scheduler' })
      .catch(err => console.error('[scheduler] Fallo al publicar feed.cleanup:', err.message));
  }, FEED_CLEANUP_INTERVAL_MS);
